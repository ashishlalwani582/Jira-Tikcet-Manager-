...
// delete walo ko save krne k liye
...
function loadTasksDel() {
  console.log("tasks are loaded");

  let tasks = JSON.parse(localStorage.getItem("delTasks"));
  for (let i = 0; i < tasks.length; i++) {
    let id = tasks[i].id;
    let color = tasks[i].color;
    let task = tasks[i].task;
    ...
    ticket.addEventListener("click", function (e) {
      ...
      let tasksArr = JSON.parse(localStorage.getItem("delTasks"));
